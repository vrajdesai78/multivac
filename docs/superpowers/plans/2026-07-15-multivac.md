...
def build_argv(req: Req, *, session_id=None, new_session_id=None, prompt=None):
    ...
    if req.cwd and tool != "codex":
        # codex uses -C; others get cwd via run_child's cwd= (they key sessions on it)
        pass
    argv.append(prompt if prompt is not None else req.prompt)
    return argv, planned
```

(Note: `--cwd` scoping is applied through `run_child(cwd=...)` for all tools; codex additionally honors it because it's launched in that directory. `-C` is not needed when the process cwd is already set.)
